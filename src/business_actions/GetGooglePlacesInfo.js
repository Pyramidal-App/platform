...
const PHONE_NUMBER_REGEX = /\+(?<countryCode>\d+)\s+(?<rawAreaCode>(9\s)?\d+)\s+(?<rawNumber>[\d|\-]+)/
...
class GetGooglePlacesInfo extends BusinessAction {
  ...
  async findOrCreatePhoneNumber(googlePlaceInfo) {
    ...
    const { groups: { countryCode, rawAreaCode, rawNumber } } = match

    const areaCode = rawAreaCode.match(ONLY_NUMBERS_REGEX).join('')
